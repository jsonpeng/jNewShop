      ...
      @if ($order->order_pay == '未支付')
        <div class="order-status weui-cell">
          <div class="weui-cell__hd">
            ...
            @if(getSettingValueByKey('order_expire_time'))
              <div class="tips">
                剩余时间 :
                <span style="padding-left: 20px;" id="left-time" data-endtime="{{ addOrderTimeByStartTime($order->created_at) }}"></span>
              </div>
            @endif
          </div>
          <div class="weui-cell__ft">
    ...
    @else
      @if ($order->order_pay == '未支付')
        <div class="checkwrapper product-checker">
          @if ($order->jifen > $user->credits)
            <span style="margin-left: 0.75rem; font-size: 0.6rem;">您账户{{ getSettingValueByKeyCache('credits_alias') }}({{ $user->credits }})不够</span> 
...
<script type="text/javascript" src="{{ asset('js/ap.js') }}"></script>
  ...
  <script type="text/javascript">
    ...
    function _fresh(){
     ...
     @if(getSettingValueByKey('order_expire_time') && $order->order_pay == '未支付' && $order->order_status != '已取消')
       $(function(){
          startShowCountDown($('#left-time').data('endtime'),'#left-time','ordersale',autoCancleOrder);
       });
     @endif

     function autoCancleOrder(){
        cancelOrderConfirm('{{$order->id}}','自动取消');
        alert('当前订单已超时,将自动取消');
