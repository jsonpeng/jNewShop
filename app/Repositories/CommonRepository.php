...
class CommonRepository
{
    ...
    /**
     * [检查身份证照片的有效性]
     * @param  [type] $img_src  [图片链接地址]
     * @param  string $img_side [默认正面front，背面请传back]
     * @return [type]           [description]
     */
    public function varifyCardImage($img_src = null,$img_side = 'front')
    {
        $host = "https://ocridcard.market.alicloudapi.com";
        $path = "/idimages";
        $method = "POST";
        $appcode = "fc643f19813d461f837287c3d6ca022e";
        $headers = array();
        array_push($headers, "Authorization:APPCODE " . $appcode);
        //根据API的要求，定义相对应的Content-Type
        array_push($headers, "Content-Type".":"."application/x-www-form-urlencoded; charset=UTF-8");
        $querys = "";
        $bodys = 'image='.$img_src.''.'&idCardSide='.$img_side.''; //图片 + 正反面参数 默认正面，背面请传back
        //或者base64
        ...
        curl_setopt($curl, CURLOPT_POSTFIELDS, $bodys);
        $out_put = json_decode(curl_exec($curl),1);
        if($out_put['code'] == 1)
        {
            return ['code'=>0,'message'=>$out_put['result']];       
        }
        else{
            return ['code'=>1,'message'=>'身份证验证失败']; 
        }
    }
