...
class CategoryController extends Controller
{
    ...
    public function level2(Request $request, $id)
    {
        ...
        foreach ($childrenCats as $key => $cat) {
            if($cat->id == $id)
            {
                // unset($childrenCats[$key]);
            }
        }
        $parent_cat = $this->categoryRepository->findWithoutFail($category->parent_id);
        return view(frontView('cat.level2'), compact('parent_cat','products', 'childrenCats', 'parent_id', 'id', 'category'));
    }
