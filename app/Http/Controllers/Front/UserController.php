...
class UserController extends Controller
{
    ...
    public function index(Request $request, $id=0){
        ...
        $order_nums_arr = app('commonRepo')->orderRepo()->userCenterOrderNum($user);
        return view( frontView('usercenter.index'), compact('user', 'userLevel','order_nums_arr') );
    }
    ...
    public function myCollections(Request $request)
    {
        ...
        return view(frontView('usercenter.mycollections.index'), compact('products'));
    }
    ...
    public function userBalancePage(){
        $user = auth('web')->user();
        $moneyLogs = $this->userRepository->moneyLogs($user, 0, 18,null,'desc');
