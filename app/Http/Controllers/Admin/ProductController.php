...
use Response;use Illuminate\Support\Facades\Input;
...
class ProductController extends AppBaseController
{
    ...
    public function create()
    {
        ...
        $wordList=Word::all();
        $selectedWords=[];
        $stores = app('commonRepo')->storeRepo()->all();
        $selectedStores = [];
        //商品产地
        $countries = $this->countryRepository->countriesArray();
    ...
    public function store(CreateProductRequest $request)
    {
        ...
        if(array_key_exists('stores', $inputs)){
              if(!empty($inputs['stores'])){
                 $product->stores()->sync($inputs['stores']);
              }
        }
    ...
    public function edit(Request $request,$id)
    {
     ...
     //  return $wordlist;
       //return $selectedWords;
        ...
        $stores = app('commonRepo')->storeRepo()->all();
        $selectedStoresArr = $product->stores()->get();
        $selectedStores = [];
        foreach ($selectedStoresArr as $key => $value) {
            array_push($selectedStores,$value->id);
        }
        //商品产地
        $countries = $this->countryRepository->countriesArray();
    ...
    public function destroy($id)
    {
        ...
        $redirectUrl = session('productIndexUrl') ? session('productIndexUrl') : route('products.index');
    ...
    public function ajaxGetSpecInput(Request $request, $product_id)
    {
        ...
        */
        ...
        foreach ($keySpecGoodsPrice_tmp as $key => $value) {
            $keySpecGoodsPrice[$value->key] = ['key_name' => $value->key_name, 'inventory' => $value->inventory,'price' => $value->price,'sku' => $value->sku,'image'=>$value->image,'jifen'=>$value->jifen];
        }
        ...
        $alias = getSettingValueByKey('credits_alias');
        ...
        foreach ($spec_arr2 as $k => $v) 
        {
           ...
           // $i++;
            ...
            $jifen = 0;
            if (array_key_exists($item_key, $keySpecGoodsPrice) && array_key_exists('jifen', $keySpecGoodsPrice[$item_key])) {
                $jifen = $keySpecGoodsPrice[$item_key]['jifen'];
            }
            $inventory = 0;
            ...
            $str .= sprintf("<td width=100><input name=item[%s][jifen] class='form-control' value='%s' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>", $item_key, $jifen);
    ...
    public function searchGoodsFrame(Request $request){
        ...
        if(array_key_exists('cat_level01',$inputs) || array_key_exists('cat_level02',$inputs) || array_key_exists('cat_level03',$inputs)){
            ...
            if ($inputs['cat_level01']!=0) {
                $inputs['category_id'] = $inputs['cat_level01'];
                $second_cats=$this->categoryRepository->getRootCatArray($inputs['category_id']);
            }

            if ($inputs['cat_level02']!=0) {
                $inputs['category_id'] = $inputs['cat_level02'];

                $third_cats=$this->categoryRepository->getRootCatArray($inputs['category_id']);
                $level02= $inputs['category_id'];
            }
            ...
            if($inputs['category_id']!=0) {
                $products = $product->whereIn('category_id', $this->categoryRepository->getCatArrById($inputs['category_id']));
            }
        }
        ...
        $product_list_get=$product->with('specs')->get();
        $products=$product->where('shelf', 1)->with('specs')->paginate($this->defaultPage());
        $i=0;
        foreach ($product_list_get as $item){
            $items=$item->specs;
            if(count($items)){
